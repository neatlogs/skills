# Step 4: Add WORKFLOW Span to User-Facing Entry Point
...
```python
# ❌ WRONG — graph node functions are NOT workflows. They are auto-traced by the instrumentor.
...
The `"langchain"` instrumentor already creates spans for:
- Every graph node execution
- Every LLM call inside nodes
- Every tool call via ToolNode
- The graph.invoke() call itself

You only need ONE `@span(kind="WORKFLOW")` at the top to:
1. Create the trace root (all auto-instrumented spans inherit this trace ID)
...
|--------------|---------|
| Graph node functions (planner_node, analyst_node) | Auto-traced by langchain instrumentor |
