# Step 6: Verify Tool Functions Are Untouched
...
The `"langchain"` instrumentor automatically traces ALL functions with `@tool` from `langchain_core.tools`. Adding `@neatlogs.span()` or `@neatlogs.trace()` creates duplicate spans that pollute the trace view.
