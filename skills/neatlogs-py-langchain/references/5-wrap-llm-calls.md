...
## Why This Is Needed Even With Auto-Instrumentation

The `"langchain"` instrumentor captures: model name, token counts, latency, finish reason.
...
The node function itself should NOT have `@neatlogs.span()`. The `"langchain"` instrumentor already traces it. You are ONLY adding the `with neatlogs.trace()` context manager INSIDE the function body, around the LLM call.
