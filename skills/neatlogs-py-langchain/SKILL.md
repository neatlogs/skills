...
- Exercise the actual user-facing chain/graph path. Inspect the full persisted span tree and attributes, not only a trace-list summary or local debug output. Confirm the marker-matched project trace is the fresh run, with one handler-owned hierarchy and no duplicate LLM/chain/node/tool spans. An offline/no-export verifier is insufficient by itself.
