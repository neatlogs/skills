...
The `"openai_agents"` instrumentor auto-traces agents, tools, handoffs, guardrails, and LLM calls. The one thing it cannot do is create the trace root. Add exactly one:
...
Without this WORKFLOW root, every auto-traced agent/tool/handoff/LLM span becomes an orphan in its own trace and nothing shows up grouped together.
...
These are declarative SDK constructs; the instrumentor reads them. A manual decorator creates duplicate spans.
...
3. `instrumentations=["openai_agents", "openai"]` is in `init()` (Step 2 / detect_stack).
