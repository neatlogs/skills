...
Use `span()` to represent real custom orchestration. Call `wrapAISDK`-owned operations directly inside it; do not add an LLM `trace()` around them. Use a manual LLM trace only for an unsupported/raw call outside the AI SDK.
