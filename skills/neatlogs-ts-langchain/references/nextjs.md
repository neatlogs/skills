...
Next.js runs `register()` once at server startup — the right place for `init()`. Your route handlers then create a `langchainHandler()` and pass it via `{ callbacks: [handler] }`; LangChain imports in those routes can be plain static imports.
...
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    await init({
      ...
      // NO instrumentations key — init() throws for "langchain".
    });
