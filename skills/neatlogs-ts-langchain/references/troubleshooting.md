...
Because the helpers patch the **instance** you pass (not the module), there is **no import-order rule** in the TypeScript SDK and no need for dynamic `import()`. See the [core mechanism](../SKILL.md#core-mechanism--langchainhandler) for the helper for each library.
...
3. **Is the client actually wrapped?** → No → Apply the matching helper from the [core mechanism](../SKILL.md#core-mechanism--langchainhandler), e.g. `const client = wrapOpenAI(new OpenAI())`. Wrapping the *return value* and then calling the **original** variable is the usual slip.
4. **Is `NEATLOGS_API_KEY` set?** → No → Set it via env var or `apiKey` param. Without it, export is **silently disabled**.
5. **Does a helper exist for this library?** → Check the [core mechanism](../SKILL.md#core-mechanism--langchainhandler). If not, the calls are raw HTTP as far as Neatlogs is concerned → add manual spans (see [`raw-http-llm.md`](raw-http-llm.md)).
...
Use the [core mechanism](../SKILL.md#core-mechanism--langchainhandler) as the source of truth. For a library with no helper, instrument the call sites manually — see [`raw-http-llm.md`](raw-http-llm.md).
